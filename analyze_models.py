...
def analyze_model(model_path, x, t):
    print(f"\n\n========== 分析開始: {model_path} ==========")
    ...
    for i in range(0, x.shape[0], batch_size):
        ...
        pred_labels.extend(np.argmax(y, axis=1))
    ...
    for p, tr in zip(pred_labels, true_labels):
        matrix[tr, p] += 1
        
    # 正解率
    acc = np.sum(pred_labels == true_labels) / len(true_labels)
    print(f"データセットに対する正解率: {acc*100:.2f}%")
    ...
    for m in mistakes[:5]:
        print(f"正解「{m['true']}」→ 誤認「{m['pred']}」: {m['count']}回")

# パス設定
DATA_PATH = r".\1_data\train\train_data.npy"
LABEL_PATH = r".\1_data\train\train_label.npy"
PICKLE_NAME = r".\pickle\katakana_model_light.pickle"
...
t_raw = np.load(LABEL_PATH)

# 前処理
x_data = x_raw.reshape(-1, 1, 28, 28)
if x_data.max() > 1.0: x_data = x_data / 255.0

# ※注意: 学習時にシャッフルして分割しているため、
# ここでは「全データ」を使って傾向を見ます。
# （学習済みデータも含まれますが、弱点傾向を知るには十分です）
...
for fname in target_files:
    analyze_model(fname, x_data, t_raw)